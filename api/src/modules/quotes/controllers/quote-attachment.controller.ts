...
export class QuoteAttachmentController {
  ...
  @ApiOperation({
    ...
    description:
      'Create a photo or URL attachment. URL attachments automatically generate QR codes. Only 1 cover photo allowed per quote.',
  })
  ...
  @ApiResponse({
    status: 400,
    description:
      'Invalid attachment type rules (e.g., url_attachment requires url field)',
  })
  async create(
    @Request() req,
    ...
    this.logger.log(
      `Creating attachment for quote ${quoteId} (type: ${dto.attachment_type})`,
    );
  ...
  async list(
    ...
    return this.attachmentService.listAttachments(req.user.tenant_id, quoteId);
  }
  ...
  @ApiOperation({
    ...
    description:
      'Bulk update order_index for multiple attachments. All attachment IDs must belong to the quote.',
  })
  @ApiParam({ name: 'quoteId', description: 'Quote UUID' })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Attachments reordered successfully',
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Update title, URL (triggers QR regeneration), grid_layout, or order_index. Cannot change attachment_type or file_id.',
  })
  @ApiParam({ name: 'quoteId', description: 'Quote UUID' })
  @ApiParam({ name: 'attachmentId', description: 'Attachment UUID' })
  @ApiResponse({
  ...
  @ApiOperation({
    ...
    description:
      'Deletes attachment and associated QR code file (if applicable)',
  })
  @ApiParam({ name: 'quoteId', description: 'Quote UUID' })
  @ApiParam({ name: 'attachmentId', description: 'Attachment UUID' })
