...
export class FunnelStageDto {
  ...
  @ApiProperty({
    example: 100.0,
    description: 'Percentage relative to total created',
  })
  percentage: number;
}
...
export class ConversionRatesDto {
  @ApiProperty({
    example: 75.5,
    description: 'Conversion rate from sent to viewed',
  })
  sent_to_viewed: number;

  @ApiProperty({
    example: 56.2,
    description: 'Conversion rate from viewed to accepted',
  })
  viewed_to_accepted: number;

  @ApiProperty({
    example: 42.5,
    description: 'Overall conversion rate (accepted / sent)',
  })
  overall: number;
}
...
export class ConversionFunnelResponseDto {
  @ApiProperty({
    type: [FunnelStageDto],
    description: 'Funnel stages with counts and percentages',
  })
