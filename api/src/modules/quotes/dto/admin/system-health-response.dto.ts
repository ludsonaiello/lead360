...
export class ApiHealthDto {
  ...
  @ApiProperty({
    example: 1543,
    description: 'Number of API requests in the last hour',
  })
...
export class PdfGenerationDto {
  @ApiProperty({
    example: 5,
    description: 'Number of pending PDF generation jobs',
  })
  queue_size: number;

  @ApiProperty({
    example: 2350.25,
    description: 'Average PDF generation time in milliseconds',
  })
  avg_generation_time_ms: number;

  @ApiProperty({
    example: 98.5,
    description: 'PDF generation success rate percentage',
  })
  success_rate: number;
}
...
export class EmailDeliveryDto {
  @ApiProperty({
    example: 12,
    description: 'Number of pending emails in queue',
  })
  queue_size: number;
  ...
  @ApiProperty({
    example: 3,
    description: 'Number of failed emails in last 24 hours',
  })
...
export class DatabaseHealthDto {
  @ApiProperty({
    example: 'good',
    enum: ['excellent', 'good', 'fair', 'degraded', 'error'],
    description: 'Database query performance status',
  })
