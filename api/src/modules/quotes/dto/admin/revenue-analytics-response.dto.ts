...
export class RevenueGroupDto {
  @ApiProperty({ example: 'vendor-uuid', description: 'Group ID (vendor or tenant)' })
  group_id: string;

  @ApiProperty({ example: 'ABC Roofing', description: 'Group name' })
  group_name: string;

  @ApiProperty({ example: 125000.5, description: 'Total revenue for this group' })
  revenue: number;
...
export class RevenueAnalyticsResponseDto {
  @ApiProperty({ example: 1234567.89, description: 'Total revenue across all groups' })
  total_revenue: number;

  @ApiProperty({ type: [RevenueGroupDto], description: 'Revenue grouped by vendor or tenant' })
  revenue_by_group: RevenueGroupDto[];

  @ApiProperty({ type: [RevenueGroupDto], description: 'Top 10 revenue sources' })
  top_revenue_sources: RevenueGroupDto[];

  @ApiProperty({ type: [RevenueTrendPointDto], description: 'Daily revenue trend' })
