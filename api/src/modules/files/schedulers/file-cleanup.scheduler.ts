...
export class FileCleanupScheduler {
  ...
  async scheduleDailyCleanup() {
    ...
    try {
      await this.fileCleanupQueue.add('daily-cleanup', {}, {
      ...
      this.logger.error(`Failed to schedule daily cleanup: ${error.message}`, error.stack);
    }
  }
  ...
  async triggerManualCleanup(tenantId: string, userId: string) {
    ...
    try {
      const job = await this.fileCleanupQueue.add('manual-cleanup', {
        tenantId,
        userId,
      }, {
        attempts: 2,
