...
describe('TenantManagementController', () => {
  ...
  describe('createTenant', () => {
    it('should create new tenant', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(service.createTenantManually).toHaveBeenCalledWith(
        createDto,
        'admin-123',
      );
    });
  ...
  describe('suspendTenant', () => {
    it('should suspend tenant', async () => {
      ...
      const result = await controller.suspendTenant(mockRequest, 'tenant-1', {
        reason: 'Payment overdue',
      });

      expect(result.is_active).toBe(false);
      expect(service.suspendTenant).toHaveBeenCalledWith(
        'tenant-1',
        'admin-123',
        'Payment overdue',
      );
