...
/**
 * Create Tenant WhatsApp Configuration DTO
 *
 * Validates input for creating a new WhatsApp configuration.
 * All credentials are validated before storage.
 *
 * WhatsApp-Specific Notes:
 * - Phone numbers can be provided with or without 'whatsapp:' prefix
 * - System will automatically add 'whatsapp:' prefix if not present
 * - Requires approved WhatsApp Business Account with Twilio
 */
export class CreateTenantWhatsAppConfigDto {
  @ApiProperty({
    description: 'Communication provider ID (must be twilio_whatsapp provider)',
