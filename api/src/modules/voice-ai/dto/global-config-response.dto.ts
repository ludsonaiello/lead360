...
export class SafeProviderInfo {
  ...
  @ApiProperty({ description: 'Provider key (e.g. deepgram, openai, cartesia)' })
  provider_key: string;
...
export class GlobalConfigResponseDto {
  ...
  @ApiProperty({ description: 'Whether the voice AI agent is globally enabled' })
  agent_enabled: boolean;

  @ApiPropertyOptional({ description: 'Default STT provider details', type: SafeProviderInfo })
  default_stt_provider: SafeProviderInfo | null;

  @ApiPropertyOptional({ description: 'Default LLM provider details', type: SafeProviderInfo })
  ...
  @ApiPropertyOptional({ description: 'JSON object with TTS provider-specific config' })
  ...
  @ApiPropertyOptional({ description: 'LiveKit server URL for agent connections' })
  livekit_url: string | null;

  @ApiPropertyOptional({ description: 'LiveKit SIP trunk URL for inbound calls' })
  livekit_sip_trunk_url: string | null;

  @ApiProperty({
    description: 'True when livekit_api_key is stored (key itself never returned)',
  })
  livekit_api_key_set: boolean;

  @ApiProperty({
    description: 'True when livekit_api_secret is stored (secret itself never returned)',
  })
  livekit_api_secret_set: boolean;

  @ApiPropertyOptional({ description: 'Preview of agent API key (last 4 chars)' })
