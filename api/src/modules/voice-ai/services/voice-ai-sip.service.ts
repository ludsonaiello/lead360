...
export class VoiceAiSipService {
  ...
  async buildSipTwiml(tenantId: string, callSid: string): Promise<string> {
    ...
    // Build the SIP URI. The query-param separator must be &amp; (not &) because
    // this string is embedded verbatim inside an XML element. When Twilio parses
    // the XML it decodes &amp; → & and passes the correct SIP URI to LiveKit.
    const sipUri = `sip:voice-ai@${livekitUrl}?tenantId=${encodeURIComponent(tenantId)}&amp;callSid=${encodeURIComponent(callSid)}`;
