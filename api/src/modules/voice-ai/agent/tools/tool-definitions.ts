...
export const AGENT_TOOLS: LlmToolDefinition[] = [
  {
    ...
    function: {
      ...
      description:
        'Check if a location is within the service area. Call before creating a lead to confirm coverage.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'find_lead',
      description:
        'Find an existing lead by their phone number. Call to check if the caller is already in the system. Returns full lead information including name, email, phone, address, and status if found.',
      parameters: {
        type: 'object',
        properties: {
          phone_number: {
  ...
  },
  {
    ...
    function: {
      name: 'transfer_call',
      description:
        'Transfer the call to a human. Use when caller requests to speak with a person or for complex issues.',
      parameters: {
        type: 'object',
        properties: {
          reason: {
          ...
          destination: {
            type: 'string',
            description: 'Department (sales, support, etc.) - optional',
          },
        },
        required: ['reason'],
      },
  ...
  },
  {
    ...
    function: {
      name: 'end_call',
      description:
        "End the call when the conversation is complete and you have said goodbye to the caller. Use this tool after you've provided all necessary information and concluded the conversation.",
      parameters: {
        type: 'object',
        properties: {
          reason: {
          ...
          notes: {
            type: 'string',
            description:
              'Optional brief notes about the call outcome (max 200 chars)',
          },
        },
        required: ['reason'],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Book an appointment for a quote visit. Call after lead is created or found. Use this to schedule a specific date/time for the appointment. If the caller has a preferred date, search availability for that date first. If no availability on preferred date or no preference given, offer next available slots.',
      parameters: {
        type: 'object',
        properties: {
          lead_id: {
          ...
          confirmed_date: {
            type: 'string',
            description:
              'Confirmed appointment date in YYYY-MM-DD format (when caller selects a slot)',
          },
          confirmed_start_time: {
            type: 'string',
            description:
              'Confirmed start time in HH:MM format (when caller selects a slot)',
          },
          notes: {
            type: 'string',
            description:
      ...
      parameters: {
        ...
        properties: {
          ...
          appointment_id: {
            type: 'string',
            description:
              'The UUID of the appointment to reschedule (provide when caller confirms new time)',
          },
          new_date: {
            type: 'string',
            description:
              'New date in YYYY-MM-DD format (provide when caller confirms new time)',
          },
          new_time: {
            type: 'string',
            description:
      ...
      parameters: {
        ...
        properties: {
          ...
          appointment_id: {
            type: 'string',
            description:
              'The UUID of the appointment to cancel (provide when caller confirms cancellation)',
          },
          reason: {
            type: 'string',
            description:
