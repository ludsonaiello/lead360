...
export class PaymentMethodRegistryController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Array of payment methods with usage data',
  })
  ...
  @ApiResponse({
    status: 400,
    description:
      'Validation error (invalid last_four, limit reached, invalid type)',
  })
  @ApiResponse({
    status: 409,
    description: 'Nickname already exists for this tenant',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Validation error (invalid last_four)',
  })
  @ApiResponse({ status: 404, description: 'Payment method not found' })
  @ApiResponse({
    status: 409,
  ...
  @ApiOperation({
    summary:
      'Delete a payment method (soft by default, permanent with ?permanent=true)',
  })
  @ApiParam({ name: 'id', description: 'Payment method UUID' })
  @ApiQuery({
    name: 'permanent',
    ...
    type: Boolean,
    description:
      'Set to true to permanently delete (only works if payment method has zero usage)',
  })
  @ApiResponse({
    status: 200,
    description: 'Payment method deactivated or permanently deleted.',
  })
  @ApiResponse({
    status: 400,
    description: 'Cannot permanently delete — payment method is in use',
  })
  @ApiResponse({ status: 404, description: 'Payment method not found' })
  async remove(
    @Request() req,
