...
[ ] Implement PATCH /admin/tenants/:id (updateTenant stub) — Controller route exists at tenant-management.controller.ts:213 with full Swagger schema but returns hardcoded "to be implemented". TenantService.update() already handles tenant-scoped updates (company_name, legal_business_name, EIN, contact info, etc.). The admin version should delegate to TenantManagementService with tenantId from URL param instead of JWT, add audit logging with platform_admin actor_type. Frontend API client (app/src/lib/api/admin.ts:247) and type (UpdateTenantDto) already exist but no component calls it yet.
[ ] Review all JobsModule email handling — Audit every service that sends emails (invites, notifications, reminders, follow-ups, etc.) and decide whether each should go through SMTP directly or route through the CommunicationModule's SendEmailService (which supports multi-provider: SMTP, Brevo, SendGrid, SES via BullMQ queue). Goal: consolidate on a single email dispatch path with template rendering, delivery tracking, and communication_event logging. Known inconsistency found in Sprint 10: UsersService uses SendEmailService but sprint spec assumed JobQueueService.