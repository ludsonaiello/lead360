/**
 * Create WhatsApp Configuration Modal
 * Allows Owner/Admin to create new WhatsApp configuration with Twilio credentials
 */
...
import { createWhatsAppConfig } from '@/lib/api/twilio-tenant';
import { getAvailableProviders } from '@/lib/api/communication';
import type { CreateWhatsAppConfigRequest } from '@/lib/types/twilio-tenant';
import type { CommunicationProvider } from '@/lib/types/communication';

interface CreateWhatsAppConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function CreateWhatsAppConfigModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateWhatsAppConfigModalProps) {
  const [loading, setLoading] = useState(false);
  const [fetchingProvider, setFetchingProvider] = useState(true);
  const [providerId, setProviderId] = useState<string>('');

  const [formData, setFormData] = useState<CreateWhatsAppConfigRequest>({
    provider_id: '',
    account_sid: '',
    auth_token: '',
    from_phone: '',
  ...
  // Fetch Twilio WhatsApp provider_id on mount
  useEffect(() => {
    if (isOpen) {
      fetchTwilioWhatsAppProvider();
    }
  }, [isOpen]);

  const fetchTwilioWhatsAppProvider = async () => {
    try {
      setFetchingProvider(true);

      // Fetch WhatsApp providers
      const providers = await getAvailableProviders({ type: 'whatsapp' });

      // Find Twilio WhatsApp provider
      const twilioWhatsAppProvider = providers.find(
        (p: CommunicationProvider) => p.provider_key === 'twilio_whatsapp'
      );

      if (!twilioWhatsAppProvider) {
        toast.error('Twilio WhatsApp provider not found. Contact your administrator.');
        onClose();
        return;
      }

      setProviderId(twilioWhatsAppProvider.id);
      setFormData(prev => ({ ...prev, provider_id: twilioWhatsAppProvider.id }));
    } catch (error: any) {
      console.error('Error fetching Twilio WhatsApp provider:', error);
      toast.error('Failed to load WhatsApp provider configuration');
      onClose();
    } finally {
  ...
  const validate = () => {
    ...
    // From Phone validation (accept with or without whatsapp: prefix)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Remove whatsapp: prefix if user added it (backend will add it)
      payload.from_phone = payload.from_phone.replace(/^whatsapp:/, '');
      ...
      const message = error.data?.message || 'Failed to create WhatsApp configuration';
      toast.error(message);

      // Handle specific errors
      if (error.status === 409) {
        setErrors({ account_sid: 'Active WhatsApp configuration already exists. Deactivate existing config first.' });
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Configure WhatsApp Provider"
      size="lg"
    >
      {fetchingProvider ? (
          ...
          <ModalContent>
            <div className="space-y-4">
              ...
              <PhoneInput
                ...
                required
                helperText="Your Twilio WhatsApp-enabled phone number (e.g., +19781234567). The 'whatsapp:' prefix will be added automatically."
                disabled={loading}
              />
