...
import type { UpdateTenantWhatsAppConfigDto, TenantWhatsAppConfig } from '@/lib/types/twilio-admin';

interface EditTenantWhatsAppConfigModalProps {
  open: boolean;
  onClose: () => void;
  config: TenantWhatsAppConfig;
  tenantName: string;
  onUpdate: (configId: string, data: UpdateTenantWhatsAppConfigDto) => Promise<void>;
}

export function EditTenantWhatsAppConfigModal({
  open,
  onClose,
  config,
  tenantName,
  onUpdate,
}: EditTenantWhatsAppConfigModalProps) {
  ...
  const handleUpdate = async () => {
    setUpdating(true);
    try {
      const updateData: UpdateTenantWhatsAppConfigDto = {
        is_active: isActive,
      };
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            Edit WhatsApp Configuration for {tenantName}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
            <div>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">
                Enable or disable this WhatsApp configuration
              </p>
            </div>
            <Switch
              id="isActive"
          ...
          {/* Phone Number */}
          <div>
            <Label htmlFor="fromPhone">From Phone Number (WhatsApp)</Label>
            <Input
              id="fromPhone"
              type="tel"
              value={fromPhone}
