...
import type { CreateTenantWhatsAppConfigDto, PhoneNumber } from '@/lib/types/twilio-admin';

interface CreateTenantWhatsAppConfigModalProps {
  open: boolean;
  onClose: () => void;
  tenantId: string;
  tenantName: string;
  availablePhoneNumbers: PhoneNumber[];
  onCreate: (data: CreateTenantWhatsAppConfigDto) => Promise<void>;
}

export function CreateTenantWhatsAppConfigModal({
  open,
  onClose,
  tenantId,
  tenantName,
  ...
  onCreate,
}: CreateTenantWhatsAppConfigModalProps) {
  ...
  const handleCreate = async () => {
    setCreating(true);
    try {
      // For WhatsApp, ensure the number has whatsapp: prefix
      const whatsappPhone = fromPhone.startsWith('whatsapp:') ? fromPhone : `whatsapp:${fromPhone}`;

      await onCreate({
        provider_type: providerType,
        from_phone: whatsappPhone,
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Create WhatsApp Config for {tenantName}</DialogTitle>
        </DialogHeader>
            ...
            <>
              <div>
                ...
                <Input
                  ...
                  placeholder="+15555555555 or whatsapp:+15555555555"
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Will automatically add whatsapp: prefix if not provided
                </p>
              </div>
              <div>
