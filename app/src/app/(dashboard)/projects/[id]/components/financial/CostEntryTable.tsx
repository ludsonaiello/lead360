    ...
    <>
      <Card className="p-6">
        ...
        {loading ? (
          ...
          <>
                  ...
                  {entries.data.map((entry) => (
                    <tr key={entry.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="py-3 px-3">
                        <Badge variant={CATEGORY_TYPE_VARIANTS[entry.category_type] || 'gray'}>
                          {entry.category_name}
                        </Badge>
                      </td>
                      <td className="py-3 px-3 text-right font-semibold text-gray-900 dark:text-white whitespace-nowrap">
                        ...
                        {entry.has_line_items && (
                          <span className="ml-1 text-xs font-normal text-gray-400" title={`${entry.line_items?.length || 0} items`}>
            ...
            {/* Pagination */}
            {(entries.meta.pages ?? 0) > 1 && (
              <div className="mt-4">
                <PaginationControls
                  currentPage={page}
                  totalPages={entries.meta.pages ?? 1}
      ...
      {deleteEntry && (
        <DeleteConfirmationModal
          ...
          title={deleteEntry.is_recurring_instance ? 'Delete Recurring Entry' : 'Delete Cost Entry'}
          message={
            deleteEntry.is_recurring_instance
              ? `This ${formatCurrency(typeof deleteEntry.amount === 'string' ? parseFloat(deleteEntry.amount) : deleteEntry.amount)} entry was auto-generated by a recurring rule. Deleting it will NOT update the recurring rule's next due date. After deleting, check the recurring rule's schedule and update the next due date if needed to avoid skipping or duplicating an occurrence. This action cannot be undone.`
              : `Are you sure you want to delete this ${formatCurrency(typeof deleteEntry.amount === 'string' ? parseFloat(deleteEntry.amount) : deleteEntry.amount)} cost entry? This action cannot be undone.`
          }
          isDeleting={deleting}
        />
      )}
